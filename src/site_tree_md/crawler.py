...
from site_tree_md.markdown import extract_title, html_to_markdown
from site_tree_md.models import CrawlSummary, CrawlTask, ManifestRecord, SaveResult
from site_tree_md.paths import binary_path, classify_content, markdown_path
...
class SiteCrawler:
    ...
    def _save_response(self, task: CrawlTask, response: requests.Response, kind: str) -> SaveResult:
        ...
        if kind == "html":
            ...
            lines.append("---\n")
            lines.append(html_to_markdown(response.text, final_url))
        ...
        return SaveResult(path=path, sha256=sha256_bytes(payload))
